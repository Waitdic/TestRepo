namespace ThirdParty.CSSuppliers.NetStorming
{
    ...
    public abstract class NetstormingBase : IThirdParty
    {
        ...
        public bool PreBook(PropertyDetails propertyDetails)
        {
            ...
            try
            {
                ...
                string nationality = _support.TPNationalityLookup(Source, propertyDetails.NationalityID);

                // build and log the request
                xmlRequest = BuildSearchRequest(propertyDetails, propertyDetails.Rooms, firstRoomDetailsSplit[14],
                    agreementId, propertyDetails.TotalPrice, cityCode, propertyDetails.ArrivalDate,
                    propertyDetails.DepartureDate, _settings.Actor(propertyDetails), _settings.User(propertyDetails),
                    _settings.Password(propertyDetails), _settings.Version(propertyDetails), nationality, hotelCode);
                ...
                var cancellationCostRequest = new NetstormingCancellationCostRequest
                {
                    Header = NetstormingSupport.Header(
                        _settings.Actor(propertyDetails),
                        _settings.User(propertyDetails),
                        _settings.Password(propertyDetails),
                        _settings.Version(propertyDetails)),
                    Query =
                    {
                        Type = "get_deadline",
                        Product = "hotel",
            ...
            finally
            {
                ...
                {
                    propertyDetails.Logs.AddNew(Source, $"{Source} Prebook Request", xmlRequest);
                }

                if (xmlResponse != null && !string.IsNullOrEmpty(xmlResponse.InnerXml))
                {
                    propertyDetails.Logs.AddNew(Source, $"{Source} Prebook Response", xmlResponse);
                }

                if (xmlCancellationCostRequest != null && !string.IsNullOrEmpty(xmlCancellationCostRequest.InnerXml))
                {
                    propertyDetails.Logs.AddNew(Source, $"{Source} Cancellation Cost Request", xmlCancellationCostRequest);
                }

                if (xmlCancellationCostResponse != null && !string.IsNullOrEmpty(xmlCancellationCostResponse.InnerXml))
                {
                    propertyDetails.Logs.AddNew(Source, $"{Source} Cancellation Cost Response", xmlCancellationCostResponse);
                }
            }
        ...
        public string Book(PropertyDetails propertyDetails)
        {
            ...
            try
            {
                ...
                string nationality = _support.TPNationalityLookup(Source, propertyDetails.NationalityID);
                xmlRequest = BuildBookRequest(propertyDetails, _settings.Actor(propertyDetails),
                    _settings.User(propertyDetails), _settings.Password(propertyDetails),
                    _settings.Version(propertyDetails), nationality, ref2Split[0], propertyDetails.ArrivalDate,
                    propertyDetails.DepartureDate, ref2Split[1], ref2Split[2], propertyDetails.TPRef1, bookingRef,
                    _settings.ReportingEmail(propertyDetails), propertyDetails.Rooms);
            ...
            finally
            {
                ...
                if (xmlRequest != null && !string.IsNullOrEmpty(xmlRequest.InnerXml))
                {
                    propertyDetails.Logs.AddNew(Source, $"{Source} Book Request", xmlRequest);
                }

                if (xmlResponse != null && !string.IsNullOrEmpty(xmlResponse.InnerXml))
                {
                    propertyDetails.Logs.AddNew(Source, $"{Source} Book Response", xmlResponse);
                }
            }
        ...
        protected virtual XmlDocument BuildSearchRequest(IThirdPartyAttributeSearch propertyDetails,
            IReadOnlyCollection<RoomDetails> rooms, string searchNumber, string agreement, decimal price, string resortCode,
            DateTime propertyArrivalDate, DateTime propertyDepartureDate, string actor, string user, string password,
            string version, string nationality, string hotelCode)
        {
