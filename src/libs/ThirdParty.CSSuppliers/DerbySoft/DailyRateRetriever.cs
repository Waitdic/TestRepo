namespace ThirdParty.CSSuppliers.DerbySoft
{
    ...
    {
        public (List<decimal>, List<decimal>) GetDailyRates(RoomRate roomRate, int totalPax,
            DateTime arrivalDate, DateTime departureDate, out decimal feeAmount)
        {
            ...
            if (roomRate.Fees != null && roomRate.Fees.Count() > 0)
            {
                ...
                feeAmount = feeCalculator.CalculateFees(roomRate.Fees.ToList(), totalPax,
                    new FeeCalculator.DateRange(arrivalDate, departureDate), roomRate.DailyCostBeforeTax.ToList());
            }
